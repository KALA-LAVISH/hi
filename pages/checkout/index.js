...
const Checkout = () => {
  ...
  const discountPriceCustomRing = decimalBalance(
    customRing.totalPrice -
      (cartData.coupon.discount / 100) * customRing.totalPrice
  );
    ...
    try {
      if (paymentMethod === "cod") {
        const data = {
          ...
          products:
            router.query.type === "custom" ? customRing : cartData.items,
          billingInfo,
          shippingInfo,
          deliveryInfo,
          paymentData: {
        ...
        router.push(
          `/checkout/${paymentMethod}?type=${
            router.query.type === "custom" ? "custom" : "regular"
          }`
        );
      }
    } catch (err) {
                ...
                <div className={classes.cart_item}>
                  <div className={classes.cart_container}>
                    ...
                    <span className={classes.cart_disc}>
                      <b>Custom ring with {customRing.name}</b>
                      {customRing.combination &&
                        customRing.combination.map(
                          (x, idx) =>
                            x !== "id" && (
        ...
        {router.query.type === "custom" ? (
          <div className={classes.price_description}>
            ...
            <span>
              ...
              {decimalBalance(customRing.totalPrice - discountPriceCustomRing)}
            </span>
            <span>
              {t("total_incl_vat")}: {currencySymbol}
              {discountPriceCustomRing + deliveryInfo.cost}
            </span>
          </div>
        ) : (
          <div className={classes.price_description}>
