...
export default function EditVarient({
  ...
  function setPrimaryItemImage(value) {
    let data = { ...variant };
    data[data.title].image = value;
    setVarient(data);
  }
  ...
  return (
    <ModalCard handleClose={() => close()} compact={false}>
      <>
        {variant.title && variant[variant.title]?.name && (
          <div className="col-12 border-bottom border-top my-2 py-2">
            ...
            <FileUpload
              ...
              updateFilesCb={(x) => setPrimaryItemImage(x[0]?.url || null)}
              multiple={false}
              smallUi={true}
              preSelectedFiles={
                variant[variant.title]?.image
                  ? genPreSelectedFile(variant[variant.title]?.image)
                  : []
              }
            />
          </div>
